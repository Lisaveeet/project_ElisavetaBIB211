...
pygame.init() # создаём "Холст" пайгейма
pygame.mouse.set_visible(False) # делаем курсор невидимым
pygame.display.set_caption("Tanks") # делаем название экрана "Tanks"
screen = pygame.display.set_mode((1000, 700)) # создаём размер экрана
clock = pygame.time.Clock() # создаём таймер для задержки 


p1 = DefaultTank(100, 100, (1, 0), (0, 255, 0)) # создаём фигуру на координатах 100px 100px с направлениями 1 по x и 0 по y."Цвет" зелёный
p1.set_controls(pygame.K_w, pygame.K_s, pygame.K_d, pygame.K_a, pygame.K_SPACE) # делаем управление w -- вверх,K -- вниз и т.д.
p2 = DefaultTank(900, 600, (-1, 0), (255, 0, 255)) # создаём фигуру на координатах 900px 600px  с направленем -1 по x и 0 по y."Цвет" пурпурный

p2.set_controls(pygame.K_UP, pygame.K_DOWN, pygame.K_RIGHT, pygame.K_LEFT, pygame.K_KP0) # аналог прошлой фигуре
...
while True:
    ...
    for event in pygame.event.get(): # если нажать QUIT -> exit
        if event.type == pygame.QUIT:
            quit()
    keys = pygame.key.get_pressed() # проверяем, какие клавиши нажаты
